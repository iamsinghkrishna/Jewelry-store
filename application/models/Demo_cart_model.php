        ...
        foreach ($this->data['cart_items'] as $k => $sData) {
            foreach ($itemId as $key => $data) {
                if ($data == $sData['id']) {
                    ...
                    $db_data = array('quantity' => $this->input->post('quantity_' . $sData['id']));
                    $this->db->where('user_id', $user_id);
                    $this->db->where('product_id', $sData['id']);
...
//        echo '<pre>', print_r($_POST);die;
        if (isset($item_id) && $item_id != '') {
            ...
            $varient_arr = array(
                    'Prebook' => $this->input->post('prebook'),
        ...
        $query = $this->db->query("select * from users_cart_items where user_id='" . $user_id . "' and product_id='" . $this->input->post('item_id') . "'");
        ...
        if ($user_id != '' && $query->num_rows() == 0) {
            $db_data = array(
                ...
                'user_id' => $user_id,
                'is_preorder' => serialize($varient_arr));
            $this->db->insert('users_cart_items', $db_data);
        } else {
            $db_data = array(
                'product_id' => $this->input->post('item_id'),
                'product_name' => $this->input->post('name'),
                'quantity' => $result[0]['quantity'] + $qty,
                'price' => $this->input->post('price'),
                'item_url' => $this->input->post('item_url'),
                'created_time' => date("Y-m-d H:i:s"),
                'user_id' => $user_id,
...
//            $user_id = 1;
        ...
        // Save cart and customer details.
        return $this->flexi_cart_admin->save_order($custom_summary_data, $custom_item_data, $this->flexi_cart->order_number());

    }

    public function retainCart() {
        $this->load->library('flexi_cart');
        $userid = $this->session->userdata('user_id');
        if ($userid != '') {
            $query = $this->db->query('select * from users_cart_items where user_id=' . $userid);

            $cart_items = $query->result_array();
            $this->data['cart_items'] = $this->flexi_cart->cart_items();
            $exising_cat_val = array();
            foreach ($this->data['cart_items'] as $key => $val) {
                array_push($exising_cat_val, $val['id']);
            }
//        echo "<pre>";
//        print_r($exising_cat_val);
            foreach ($cart_items as $value) {
                 foreach (unserialize($value['is_preorder']) as $key => $val) {
                   $varient_arr[$key] = $val;
                }
                if (!in_array($value['product_id'], $exising_cat_val)) {
                    $cart_data = array(
                        'id' => $value['product_id'],
