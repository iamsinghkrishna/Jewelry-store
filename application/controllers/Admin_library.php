...
//        $this->data['cart_items'] = $this->flexi_cart->cart_items();
		...
		$prebook = (unserialize($this->data['item_data'][0]['ord_det_item_option']));
                                                    //print_r($prebook);
        if(count($prebook) > 0 && $prebook['Prebook'] == 'true'){
            $this->data['advance'] = ' ( Advance order )';
        }else{
           $this->data['advance'] = '';  
        }
...
//        echo $html;die; 
    ...
    function all_order_details($order_number) {
        ...
        $sql_where = array($this->flexi_cart_admin->db_column('order_details', 'order_number') => $order_number);
        $this->data['item_data'] = $this->flexi_cart_admin->get_db_order_detail_array(FALSE, $sql_where);
       foreach($this->data['item_data']  as $key=>$value){
            $this->data['item_data'][$key]['product_details'] = end($this->common_model->getRecords('it_products','*',array('id'=>$value['ord_det_item_fk'])));
        }
        // Get an array of all order statuses that can be set for an order.
        // The data is then to be displayed via a html select input to allow the user to update the orders status.
        $this->data['status_data'] = $this->flexi_cart_admin->get_db_order_status_array();
...
//        );
    ...
    function summary_discounts() {
        ...
        $this->data['discount_data'] = array_reverse($this->flexi_cart_admin->get_db_discount_array(FALSE, $sql_where));
		//echo '<pre>';
        //print_r($this->data['discount_data']);die();
