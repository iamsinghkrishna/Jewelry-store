...
class Standard_library extends MX_Controller {
...
//error_reporting(E_ALL);
        ...
        if ($this->input->post('update')) {
            ...
            $coupon_name = $this->input->post('discount');
            //echo '<pre>';print_r($coupon_name);
           $current_coupon_name = $this->session->userdata('coupon_name');
            if ($current_coupon_name != '') {
                $conditions = array("disc_code" => $current_coupon_name);
                $conditions_new = array("coupon_name" => $current_coupon_name, "user_id_fk" => $user_id);
                $coupon_code_name = $current_coupon_name;
            }else{
                $conditions = array("disc_code" => $coupon_name[0]);
                $conditions_new = array("coupon_name" => $coupon_name[0], "user_id_fk" => $user_id);
                $coupon_code_name = $coupon_name[0];
            }
            //echo '<pre>';print_R($conditions);die;
            $arr_coupon_available_details = $this->common_model->getRecords('discounts', '*', $conditions);
            //echo '<pre>';print_R($arr_coupon_available_details);die;
            if ($coupon_code_name != '' && count($arr_coupon_available_details) > 0) {

                $arr_coupon_details = $this->common_model->getRecords('coupon_applied_details', '*', $conditions_new);
                //echo '<pre>';print_r($arr_coupon_details);die;
                if ($user_id != '' && ($arr_coupon_details) < 1) {
                    $coupon_name = $coupon_code_name;
                    $this->session->unset_userdata('coupon_name');
                    $this->session->set_userdata("coupon_name", $coupon_name);

                    $this->update_cart();
                } else {
                    $msg = 'You have already applied this coupon ,please enter another coupon code';
                    $this->session->unset_userdata('message');
                    $this->session->set_userdata("message", $msg);
                }
            } else {
                $msg = 'This coupon is expired dd';
                $this->session->unset_userdata('message');
                $this->session->set_userdata("message", $msg);
            }
        }
        // Update discount codes.
            ...
            //echo '<pre>';print_r($coupon_name);
            $current_coupon_name = $this->session->userdata('coupon_name');
            $current_coupon_code_main = '';
            if($current_coupon_name != ''){
                $current_coupon_code_main =$coupon_name[$current_coupon_name];
            }
            //echo $current_coupon_code_main;die;
            if ($current_coupon_name != '') {
                $conditions = array("disc_code" => $current_coupon_code_main);
                $conditions_new = array("coupon_name" => $current_coupon_code_main, "user_id_fk" => $user_id);
                $coupon_code_name = $current_coupon_code_main;
            } else {
                $conditions = array("disc_code" => $coupon_name[0]);
                $conditions_new = array("coupon_name" => $coupon_name[0], "user_id_fk" => $user_id);
                $coupon_code_name = $coupon_name[0];
            ...
            //echo '<pre>';print_R($conditions);die;
            $arr_coupon_available_details = $this->common_model->getRecords('discounts', '*', $conditions);
            //echo '<pre>';print_R($arr_coupon_available_details);
            if ($coupon_code_name != '' && count($arr_coupon_available_details) > 0) {
                //echo $arr_coupon_available_details[0]['disc_usage_limit'];die;
                if($arr_coupon_available_details[0]['disc_usage_limit'] > 0){
//                if ($arr_coupon_available_details[0]['disc_usage_limit'] == '0') {
...
//                } else {
                $arr_coupon_details = $this->common_model->getRecords('coupon_applied_details', '*', $conditions_new);
                //echo '<pre>';print_r($arr_coupon_details);die;
                if ($user_id != '' && count($arr_coupon_details) < 1) {
                    //$msg = 'This coupon is expired.';
                    //$this->session->unset_userdata('message');
                    //$this->session->set_userdata("message", $msg);
                    $coupon_name = $coupon_code_name;
                    $this->session->unset_userdata('coupon_name');
                    $this->session->set_userdata("coupon_name", $coupon_name);
//                        print_r($coupon_name);die;
                    $this->update_discount_codes($coupon_name);
                } else {
                    $msg = 'You have already applied this coupon ,please enter another coupon code';
                    $this->session->unset_userdata('message');
                    $this->session->set_userdata("message", $msg);
...
//                }
                    ...
                    $msg = "$coupon_code_name coupon exceed the usage limit,please enter another coupon.";
                    $this->session->unset_userdata('message');
                    $this->session->set_userdata("message", $msg);
                }
            } else {
                $msg = 'Please enter correct coupon code';
                $this->session->unset_userdata('message');
                $this->session->set_userdata("message", $msg);
            }
        }
...
//return true;
     ...
     */
    function delete_item($row_id = FALSE, $original_pr_id = FALSE) {
        $user_id = $this->session->userdata('user_id');
        $this->db->where('user_id', $user_id);
        $this->db->where('product_id', $original_pr_id);
...
//        redirect('standard_library/view_cart');
     ...
     */
    function update_discount_codes($coupon = "") {

        // Get the discount codes from the submitted POST data.
        $discount_data = $this->input->post('discount');


        if ($discount_data[$coupon] == '') {
            $msg = 'This coupon is expired.';
            $this->session->unset_userdata('messageNew');
            $this->session->set_userdata("messageNew", $msg);
        }
        if ($discount_data[$coupon] != '') {
            $discval = $this->common_model->getRecords('discounts', 'disc_usage_limit', array('disc_code' => $discount_data[$coupon]));

            if ($discval[0]['disc_usage_limit'] < 1) {
                $msg = 'This coupon is exceeded the usage limit.';
                $this->session->unset_userdata('message');
                $this->session->set_userdata("message", $msg);
                $this->session->unset_userdata('coupon_name');
            } else {
...
//        print_r($this->flexi_cart->get_messages());die;
        redirect('standard_library/view_cart');
    }
    ...
    function unset_discount($discount_id = FALSE) {
        ...
        $this->session->set_userdata('coupon_code_use_limit', 1);
        $coupon_code_use_limit_name = $this->session->userdata('coupon_name');
        $this->session->set_userdata('coupon_code_use_limit_name', $coupon_code_use_limit_name);
