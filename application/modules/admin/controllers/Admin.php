...
//ini_set('max_input_vars', 10000);
        ...
        $this->load->model(array('users', 'backend/product_category', 'backend/product_sub_category','Ion_auth_model'));
...
// redirect them to the login page
        ...
        $this->data['complete_sell']       = $this->common_model->getRecords('order_summary',
            'sum(ord_total) as total_sell', array('ord_status'=>'4'));
        ...
        $this->data['nz'] = $this->common_model->getRecords('tbl_trans_newsletter_subscription','count(*) as users',array('subscribe_status'=>'Active'));
...
//                        if(count($arr_product_details) < 1){
                            ...
                            $dataProductArray['initial_letter']       = trim($rowData['S']);
                            $dataProductArray['initial_color']        = trim($rowData['T']);
                            $dataProductArray['initial_mm']           = trim($rowData['U']);
...
//echo '<pre>';print_r($product_category);die;
            $dataProductArray = array(
                ...
                'clover_id' => $this->input->post('clover_id'),
                ...
                'initial_color' => $this->input->post('initial_color'),
                'initial_mm' => $this->input->post('initial_mm'),
            );
//echo '<pre>';print_R($dataProductArray);die;
            $productId        = $this->product->insert($dataProductArray);
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $method === 'addcsvdata') {
            ...
            for ($i = 0; $i < $csv_data_count; $i++) {
                ...
                if (count($arr_product_details) < 1) {
                    ...
                    $dataProductArray['createddate']          = date('Y-m-d H:m:s');
//                    echo '<pre>';
//                    print_r($dataProductArray);
//                    die;
                    $productId                                = $this->product->insert($dataProductArray);

                    $product_sku_old = trim($rowData['A']);
                    if ($product_sku_old != '') {
                        $arr_product_details = end($this->common_model->getRecords('it_products_bk',
                                'clover_id',
                                array('product_sku' => $product_sku_old)));
...
//echo '<pre>';print_r($dataProductUpdateArray);die;
            ...
            /* update quantity on clvoer */
            $this->load->library('clover');
            $quantity = $this->common_model->getRecords('it_products',
                    'quantity,clover_id', array('id' => $productId));
                $this->clover->updateStockCount($quantity[0]['clover_id'],
                    array('quantity' => $this->input->post('product_quantity'))); 
...
//}
        ...
        $this->data['curr_cat_products1'] = $this->product->get_product_by_category_id($this->data['curr_data'][0]['category_id'],
            $this->data['curr_data'][0]['subcategory_id'], 0, 1000, '', '');
...
//                die;
        ...
        $this->data['curr_cat_products2'] = $this->product->get_product_by_category_id($this->data['curr_data'][1]['category_id'],
            $this->data['curr_data'][1]['subcategory_id'], 0, 1000, '', '');
...
//                die;
        ...
        $this->data['curr_cat_products3'] = $this->product->get_product_by_category_id($this->data['curr_data'][2]['category_id'],
            $this->data['curr_data'][2]['subcategory_id'], 0, 1000, '', '');
...
//                die;
    ...
    public function getProductBySubCategory()
    {
        $product_details = $this->product->get_product_by_category_id($this->input->post('product_category_id'),
            $this->input->post('product_sub_category_id'), 0, 1000, '', '');
        $output          = "";
...
//echo '<pre>';echo count($sub_category);die;
        if (count($sub_category) > 0) {
//            foreach ($sub_category as $category) {
                
                $attribute_ids            = $sub_category;
                $conditions               = "id = '$sub_category'"; //get subcategory
                $arr_sub_category_details = $this->Common_model_marketing->getRecords('p_attributes',
                    'attrubute_value,id', $conditions);

                $conditions     = "attribute_id = '$sub_category'";
                $arr_code_check = $this->Common_model_marketing->getRecords('mst_sub_attributes',
                    'id,name', $conditions);
                ...
                if (count($arr_code_check) > 0) {
                    ...
                    <?php foreach ($arr_code_check as $key => $sub_attributes) { ?>
                        <div class="div_<?php echo $this->input->post('sub_category')?>">
...
//$mail = $this->common_model->sendEmail(array($email), array("email" => 'webmaster@rebelutedigital.com', "name" => config_item('website_name')),$subject2,$message2);
            ...
            $mail = $this->common_model->sendEmail(config_item('admin_email'),
                array("email" => config_item('admin_email'), "name" => config_item('website_name')),
    ...
    public function changeOrderStatus()
    {
        if ($this->input->post('order_id')) {
            ...
            if ($this->input->post('ord_status') == 3) {
                ...
                $ordesr_status = '<button type="button" class="btn btn-danger btn-xs">Shipped</button>';
                $html          = $this->load->view('email_templates/order_shipped',
                    $data, TRUE);
                $subject       = "Order Shipment Created";
            }
...
//        }
    ...
    {
        $pr = $this->common_model->getRecords('it_products', '*');
        foreach ($pr as $prr) {

            if ($prr['product_sku'] != '') {
                $arr_product_details = end($this->common_model->getRecords('it_products_bk',
                        'clover_id', array('product_sku' => $prr['product_sku'])));

                if (count($arr_product_details) > 0) {
                    $arr_to_update   = array("clover_id" => $arr_product_details['clover_id']);
                    $condition_array = array('product_sku' => $prr['product_sku']);
                    $this->common_model->updateRow('it_products',
                        $arr_to_update, $condition_array);
                }
            }
    ...
    {
        if ($this->input->post('order_tracking_number') != '') {
            $update = $this->common_model->updateRow('order_summary',
                array('order_tracking_number' => str_replace(' ','',$this->input->post('order_tracking_number'))),
                array('ord_order_number' => $this->input->post('order_number')));
            echo json_encode(array('status' => 200, 'msg' => 'Order tracking number updated'));
        } else {
            echo json_encode(array('status' => 201, 'msg' => 'Please enter order tracking number'));
        }
    }
    public function update_bulk_inventory()
    {
        $this->load->library('clover');
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if (($_FILES["userfile"])) {
...
//header will/should be in row 1 only. of course this can be modified to suit your need.
                ...
                if (!empty($excel_row_data)) {
                    foreach ($excel_row_data as $rowData) {
                        if ($rowData['A'] != '') {
                            $sku       = trim($rowData['A']);
                            $quantity  = trim($rowData['C']);
                            $clover_id  = trim($rowData['D']);
                            $productId = $this->common_model->updateRow('it_products',
                                array('quantity' => $quantity),
                                array('product_sku' => $sku));
                            if($clover_id !=''){
//                            $this->clover->updateStockCount($clover_id,
//                    array('quantity' => $quantity));
                            }
                        }
                    }
...
//echo '<pre>';print_r($this->data['arr_message_details'] );die;
        ...
        $this->template->write_view('header', 'backend/header', $this->data);
        $this->template->write_view('sidebar', 'backend/sidebar', NULL);
        $this->data['page_title']          = 'Update Bulk Inventory';
        $this->template->write_view('content',
            'mng_product/_update_bulk_inventory', $this->data);
        $this->template->write_view('footer', 'backend/footer', '', TRUE);
        $this->template->render();
    }
    ...
    public function changePassword(){
        ...
        if($this->input->post()){
            ...
            if ($this->form_validation->run() == true)
                {
                    $hash_password = $this->Ion_auth_model->hash_password($this->input->post('password'));
                   $this->common_model->updateRow('users',array('password'=>$hash_password),array('id'=>$user_id));
...
//echo '<pre>';print_r($this->data['arr_message_details'] );die;
        ...
        $this->template->write_view('sidebar', 'backend/sidebar', NULL);
        $this->data['page_title']          = 'Contact Message List';
        $this->template->write_view('content', 'change_password',
            $this->data);
        $this->template->write_view('footer', 'backend/footer', '', TRUE);
        $this->template->render();
    }
